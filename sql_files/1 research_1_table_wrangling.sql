/* In cleaning data, I like to look at all the columns first and write the expected data type 
 and their expected constraints. After writing them down, I start checking if the columns are 
 up to my expectations. I also want to rename the columns so that it's more standardized and 
 has more context. */
...
-- Now, we're going to change the column names in the duplicate table to make them easier to use later in analysis:
...
/* We have 720 respondents in the file. Upon closer inspection, the id in this file skips mulitiple numbers, 
making the id confusing. We'll deal with that later after checking for duplicates. */

/* The next thing I want to do is to check for duplicates. In this particular case, we would only call 
something a duplicate if there is an match in all fields. To do that, we will use the ROW_NUMBER window 
function to check for duplicates. So we need to put all the columns in the PARTITION BY clause to check for that.*/
...
/* Now that we have checked that the id's are now properly replaced, we proceed to splitting this 
huge table into multiple tables. The questionnaire used for this survey is split into 6 parts, namely 
the Demographics Questionnaire, Life of Faith & Church Life Questionnaire, Love Language 
Questionnaire, Leader Qualities Questionnaire, and Satisfaction Questionnaire. I'm going to split this 
into 6 tables, including a table where some of the Questionnaire Questions are placed so we can 
reference them in the visualization later.
*/
