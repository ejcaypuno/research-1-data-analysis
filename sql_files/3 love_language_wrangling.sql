/* Right now, our table is too wide. I want to make it a narrow table
so it's easier to work with. Here, we're going to pivot our table so that we 
only have a few columns and our observations will be on our rows. */
...
/* The love languages portion of the questionnaire requires the respondent to rank several statements based 
on the importance. Since it's a forced-ranking response, there should be no repeating numbers witin each 
situation presented. We're going to check which ids have valid responses by checking if the total of 
scores is 15 (i.e. 1 + 2 + 3 + 4 + 5) and checking if the responses are distinct from one another within 
a situation. */
...
/* We then filter out those that do not add up to our expected values, which is a sum of 15 (i.e. 1+2+3+4+5) 
 * and a count of 5 (i.e. 1, 2, 3, 4, 5; no repetitions) */
score_check_2 AS (
SELECT 
	id
    ,score_sum
...
/* Since there are multiple situations and leader types, we need to get our expected values in all of the 
situations and leader types to consider them as valid.*/
...
/* There are 429 invalid responses. Adding the 291 valid responses gives us 720, which means that 
our filtering system worked. I want this information to be permanent and be reflected in the table 
so I'm going to make a column that ties the id whether it's valid or not */
ALTER TABLE love_language_scores
ADD valid_check VARCHAR(255) NOT NULL;


/* Using the list of valid ids we obtained above, we're going to fill the newly made column 
...
--  The output is 429, which is correct.


/*
Now you might be asking "Why don't you just eliminate the invalid responses from the table?". 
This is because I'm interested in seeing the differences in results between a likert scale 
and a ranking scale. I'm going to do an analysis on both and see if the results are the same 
